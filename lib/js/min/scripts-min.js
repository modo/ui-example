...
function accountSetting(){
	...
	setting.me 					= $(this);
	setting.head 				= $('.section-header', setting.me);
	
	setting.trigger 			= {};
	setting.trigger.me 			= $('a', setting.head);
	setting.trigger.closeText 	= setting.trigger.me.text();
	setting.trigger.openText 	= 'hide';
	
	if(setting.trigger.closeText == 'deactivate') setting.trigger.openText = 'cancel';
	
	setting.edit 				= $('.edit', setting.me);
	setting.summary 			= $('.summary', setting.me);
	
	setting.status				= 'closed';
	
	setting.edit.hide();
	setting.head.add(setting.summary).css('cursor', 'pointer').click(function(){
		setting.trigger.me.click();
	});
	
	$('a', setting.me).on('click', function(e){ e.preventDefault(); });
	
	setting.trigger.me.click(function(){
		if(setting.status == 'closed') editSetting();
		if(setting.status == 'open') closeSetting();
	});
	
	$('button', setting.edit).click(function(){
		window.location.href=window.location.href
	});
	
	function editSetting(){
		setting.summary.slideUp(250, function(){
			setting.edit.slideDown(250, function(){
				setting.status = 'open';
			})
		});
		
		setting.trigger.me.text(setting.trigger.openText);
	}
	
	function closeSetting(){
		setting.edit.slideUp(250, function(){
			setting.summary.slideDown(250, function(){
				setting.status = 'closed';
			})
		});
		
		setting.trigger.me.text(setting.trigger.closeText);
	}
}
...
// Import base scripts
// @codekit-prepend '../base/js/scripts.js'

// USGBC-specific functions
// ---------------------------------------------
// @codekit-prepend 'subnav.js';
// @codekit-prepend 'fix-select.js';
// @codekit-prepend 'account-settings.js';
...
>>>>>>> 62f68ce25dd2b92ede5b188445d15a93916a7d2b
